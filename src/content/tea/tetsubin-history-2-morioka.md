---
title: "Tetsubin History, Part 2: Morioka and Nambu Ironware"
pubDate: "2026-08-04"
category:
  - "Tea"
  - "Teapots"
  - "Tetsubin"
  - "Japanese Craft"
description: "Four Morioka workshops show how one regional tradition can produce very different kettles, patterns, and marks."
heroImage: "../../../public/tea/posts/tetsubin-history-2-morioka/hero.png"
---

[Part 1](/tea/tetsubin-history-1-birth-of-the-iron-kettle) introduced the main tetsubin regions of Japan. This part visits Morioka in Iwate Prefecture and four workshops that still shape its ironware.

<figure class="figure-center">
  <a href="/tea/posts/tetsubin-history/japan-regional-centers.svg" target="_blank" rel="noopener noreferrer" aria-label="Open the map of Japan's main tetsubin regions full size in a new tab">
    <img src="/tea/posts/tetsubin-history/japan-regional-centers.svg" alt="Map of Japan marking Morioka, Mizusawa in Oshu, Yamagata, Takaoka, and Kyoto-Omi" loading="lazy" decoding="async" />
  </a>
  <figcaption>Morioka is the northernmost workshop center in this series. It shares the modern Nambu ironware name with Mizusawa, farther south in Iwate.</figcaption>
</figure>

## Morioka and Nambu Ironware

Morioka's casting history reaches back about four hundred years. The Nambu domain brought skilled metalworkers to the city to make bells, Buddhist objects, household goods, and kettles.

The starting date is not completely clear. The [Suzuki workshop](https://suzukimorihisa.com/en/pages/nanbu-ironware-and-the-suzuki-family) and [Morioka City](https://www.city.morioka.iwate.jp/_res/projects/default_project/_page_/001/024/890/2syou.pdf) give 1625. A [2019 Iwate University paper](https://www.jstage.jst.go.jp/article/sfj/70/5/70_255/_pdf/-char/ja) gives 1641. The dates may describe different stages of the Suzuki family's arrival and service.

The tetsubin came later. Morioka tradition says the third Koizumi Nizaemon made a smaller tea kettle with a handle and spout around 1750. This is an important local tradition, but it does not prove that Morioka made the first tetsubin in Japan.

Today Morioka is one of two main production areas covered by the Nambu ironware name. The other is Mizusawa, now part of Ōshū. The name joins them today, but their older histories are different.

## How Morioka Makes a Kettle

A tetsubin begins with a mould, not with a solid block of iron. The maker shapes an outer mould and presses the surface pattern into it. A separate core creates the hollow inside. The narrow gap between the two becomes the kettle wall.

The spout is moulded separately and joined before the pour. Molten iron fills the empty space. After it cools, the mould is opened or broken. The rough casting is cleaned, checked, heated inside, colored outside, and fitted with a lid and forged handle.

The [Nambu ironware association](https://www.ginga.or.jp/nanbu/making.html) describes more than eighty operations. Workshops divide that work in different ways. Some pass the kettle between specialists. Others keep most of the process with one craftsperson. That difference matters when comparing the four workshops below.

## Suzuki Morihisa: The Domain Lineage

Suzuki Morihisa works in central Morioka. It has the closest link to the old Nambu domain among the four workshops in this article.

The workshop traces its service to 1625, while the Iwate University paper gives 1641. The hereditary name *Morihisa* came later, after the Meiji Restoration. The old shop burned in 1884 and moved the next year.

The current head is [Suzuki Morihisa Shigeo, the sixteenth](https://suzukimorihisa.com/en/pages/introduction-of-the-master). He entered production in 2008 and took the name in 2023. The head designs and supervises, while a team carries the kettles through moulding, casting, and finishing.

Suzuki Morihisa is special because it combines a long domain lineage with active design. Its [catalogue](https://suzukimorihisa.com/en/collections/all) includes arare dots and fine ribs, but also broad grooves, flowers, and playful geometry. The round *Hinomaru* and woven-looking *Temari* show that the workshop does not repeat one old shape.

<figure class="figure-center">
  <div class="video-embed">
    <iframe src="https://www.youtube.com/embed/B_3B5q2kBgk" title="Suzuki Morihisa Studio — Pen magazine feature" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
  </div>
  <figcaption>The video introduces the sixteenth Suzuki Morihisa and shows how the workshop brings modern design into its long casting tradition.</figcaption>
</figure>

A surface pattern can suggest Suzuki Morihisa, but it cannot prove the workshop. Look for an exact model match and then check the body mark. Published examples show <code>盛久</code>, read *Morihisa*, pressed into the body with a wooden seal. It is often placed low near the spout. Horie's paper shows different forms linked to the thirteenth and fourteenth heads.

<div class="image-gallery studio-gallery" role="group" aria-label="Suzuki Morihisa tetsubin and maker marks">
  <figure class="full-size">
    <a href="/tea/posts/tetsubin-history-2-morioka/suzuki-tetsubin.jpg" target="_blank" rel="noopener noreferrer" aria-label="Open the Suzuki Morihisa Hinomaru tetsubin image full size in a new tab">
      <img src="/tea/posts/tetsubin-history-2-morioka/suzuki-tetsubin.jpg" alt="Suzuki Morihisa Hinomaru tetsubin" loading="lazy" decoding="async" />
    </a>
    <figcaption>The small <em>Hinomaru</em>, designed by the thirteenth Suzuki Morihisa. Photo: <a href="https://suzukimorihisa.com/en/products/%E6%97%A5%E3%81%AE%E4%B8%B8%E5%BD%A2%E9%89%84%E7%93%B6-%E5%B0%8F">Suzuki Morihisa Studio</a>.</figcaption>
  </figure>
  <figure class="full-size">
    <a href="/tea/posts/tetsubin-history-2-morioka/suzuki-mark.jpg" target="_blank" rel="noopener noreferrer" aria-label="Open the published Suzuki Morihisa marks image full size in a new tab">
      <img src="/tea/posts/tetsubin-history-2-morioka/suzuki-mark.jpg" alt="Published Suzuki Morihisa marks reading Morihisa" loading="lazy" decoding="async" />
    </a>
    <figcaption>Published <code>盛久</code> marks linked to the thirteenth head (left) and fourteenth (right). Source: <a href="https://www.jstage.jst.go.jp/article/sfj/70/5/70_255/_pdf/-char/ja">Horie 2019, fig. 7</a>.</figcaption>
  </figure>
</div>

## Kunzan: A Branch Becomes Its Own Workshop

Kunzan is based at Morioka Handi-Works Square. It grew from apprenticeship rather than an unbroken family line.

The [workshop history](https://nanbu93.jp/pages/company) begins with Asada Kaoru, born in 1904. He trained under Suzuki Kyūtarō at Suzuki Shuzendō and opened his own workshop in 1937. Sasaki Kazuo started training in 1972, became the third Kunzan in 1976, and was certified as a traditional craftsperson in 1994.

Kunzan says it makes its patterns by hand, fires and breaks its moulds, and keeps casting in-house. That makes the workshop smaller and more self-contained than a large factory.

Its [catalogue](https://nanbu93.jp/collections/all) is broad. Fine and coarse arare appear beside rough skin, horizontal lines, tortoiseshell shapes, flowers, and flatter bodies. The *Natsume Sakura* below combines a tea-caddy shape with cherry-blossom decoration.

There is no single Kunzan pattern. An exact catalogue match is more useful than a general resemblance. Reported marks include <code>薫山</code>, read *Kunzan*, and sometimes <code>南部盛岡</code>, or *Nambu Morioka*. Kunzan does not publish a full guide to its old marks, so a box, receipt, or workshop confirmation remains important.

<div class="image-gallery studio-gallery" role="group" aria-label="Kunzan tetsubin and maker mark">
  <figure class="full-size">
    <a href="/tea/posts/tetsubin-history-2-morioka/kunzan-tetsubin.jpg" target="_blank" rel="noopener noreferrer" aria-label="Open the Kunzan Natsume Sakura tetsubin image full size in a new tab">
      <img src="/tea/posts/tetsubin-history-2-morioka/kunzan-tetsubin.jpg" alt="Kunzan Natsume-form tetsubin with cherry-blossom decoration" loading="lazy" decoding="async" />
    </a>
    <figcaption>The 1.8-litre <em>Natsume Sakura</em>. Photo: <a href="https://nanbu93.jp/products/natsume-gata-sakura-1-8l">Kunzan</a>.</figcaption>
  </figure>
  <figure class="full-size">
    <a href="/tea/posts/tetsubin-history-2-morioka/kunzan-mark.jpg" target="_blank" rel="noopener noreferrer" aria-label="Open the reported Kunzan maker mark image full size in a new tab">
      <img src="/tea/posts/tetsubin-history-2-morioka/kunzan-mark.jpg" alt="Reported Kunzan body mark beneath a tetsubin" loading="lazy" decoding="async" />
    </a>
    <figcaption>A reported <code>薫山</code> body mark. Photo: <a href="https://hurumono.net/2024/04/03/buy-tetsubinn/">Hurumono</a>. Use it as a comparison, not as a complete dating guide.</figcaption>
  </figure>
</div>

## Iwachu: Craft at Industrial Scale

Iwachu is a large Morioka maker. Its scale sets it apart from the smaller workshops.

Its English pages give several starting dates. The [company page](https://iwachu.co.jp/en/pages/about-us) says 1901 in one place and 1908 in another. The [history page](https://iwachu.co.jp/en/pages/history) says Iwashimizu Sueyoshi opened the business in 1902 after four years of training. The safest short description is “early twentieth century.”

The company became Iwashimizu Foundry in 1950 and expanded its factory in the early 1970s. It now makes tetsubin, enamelled teapots, cookware, and other cast goods.

What makes Iwachu special is that it uses different production routes. Its [traditional tetsubin process](https://iwachu.co.jp/en/pages/manufacturing-process) uses fired moulds, hand-pressed patterns, an oxide-treated interior, and exterior lacquer. Its [teapot and kitchenware process](https://iwachu.co.jp/en/pages/teapot-kitchenware-process) can include automated moulding and enamel.

<figure class="figure-center">
  <div class="video-embed">
    <iframe src="https://www.youtube.com/embed/o6AuxztRkYM" title="Iwachu ironware production" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
  </div>
  <figcaption>This overview shows how Iwachu ironware moves from mould preparation and casting to finishing.</figcaption>
</figure>

This difference also helps identification. An unenameled interior points toward a water-boiling tetsubin. A smooth enamelled interior points toward a teapot that should not go over direct flame. On the outside, Iwachu uses arare, lines, floral relief, and many modern colors. The pattern alone is not enough.

Iwachu publishes several current <code>岩鋳</code> and <code>IWACHU</code> marks. The rectangular Japanese stamp below is one official form. The company does not provide a full date range for every mark, so use the stamp together with the model, interior, and catalogue.

<div class="image-gallery studio-gallery" role="group" aria-label="Iwachu tetsubin and maker stamp">
  <figure class="full-size">
    <a href="/tea/posts/tetsubin-history-2-morioka/iwachu-tetsubin.jpg" target="_blank" rel="noopener noreferrer" aria-label="Open the Iwachu model 11001 tetsubin image full size in a new tab">
      <img src="/tea/posts/tetsubin-history-2-morioka/iwachu-tetsubin.jpg" alt="Iwachu 23-type Nambu-form arare tetsubin" loading="lazy" decoding="async" />
    </a>
    <figcaption>Iwachu model 11001, a 23-type Nambu-form arare tetsubin. Photo: <a href="https://iwachu.co.jp/en/products/11001">Iwachu</a>.</figcaption>
  </figure>
  <figure class="full-size">
    <a href="/tea/posts/tetsubin-history-2-morioka/iwachu-mark.png" target="_blank" rel="noopener noreferrer" aria-label="Open the Iwachu maker stamp image full size in a new tab">
      <img src="/tea/posts/tetsubin-history-2-morioka/iwachu-mark.png" alt="Official Iwachu rectangular stamp reading Iwachu in Japanese" loading="lazy" decoding="async" />
    </a>
    <figcaption>One official <code>岩鋳</code> stamp. Photo: <a href="https://iwachu.co.jp/en/pages/about-us">Iwachu</a>.</figcaption>
  </figure>
</div>

## Kamasada: One Maker, One Kettle

Kamasada is a small workshop in Morioka's Konyachō district. It is known for giving one maker control over much of the kettle-making process.

Its roots lie in the Miya casting family. A [Morioka City history](https://www.city.morioka.iwate.jp/_res/projects/default_project/_page_/001/024/890/2syou.pdf) records Miya Gonjirō as a caster in the early Meiji period. His son Miya Sadakichi later opened a kettle shop. The name Kamasada came with the second head, Miya Shōtarō.

The workshop was dismantled under wartime controls in 1945. It reopened in 1954 using timbers from the earlier building.

Modern Kamasada work is closely linked to Miya Nobuho. Its special quality is restraint: low round bodies, wide handles, simple lines, and rough yuzu-like surfaces. A [Japan Economic Foundation profile](https://www.jef.or.jp/journal/pdf/172th_Japanese_Craft.pdf) and a [Toraya exhibition account](https://assets.toraya-group.co.jp/__/_files/toraya/pdf/press_release/20160818_252.pdf) divide the steps differently, but both describe one craftsperson carrying a kettle through much of the process.

The [Toramonten guide](https://toramonten.com/en-eu/blogs/column/nanbu-ironware-tetsubin-guide) says current Kamasada kettles use fired moulds and a burn-off treatment inside. Those details help explain the work, but they are not a shortcut for identifying an old kettle.

<figure class="figure-center">
  <div class="video-embed">
    <iframe src="https://www.youtube.com/embed/5DOeuBQnSVw" title="Kamasada tetsubin production" loading="lazy" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
  </div>
  <figcaption>This documentary follows a Kamasada tetsubin through mould making, casting, and finishing.</figcaption>
</figure>

Rough skin is strongly associated with Kamasada, yet other workshops use it too. Look for the complete shape and an exact model match. Reported marks include <code>南部 釜定</code> below the spout and <code>KAMASADA</code> with a year beneath the lid on a modern line. There is no published guide covering every historical mark.

<div class="image-gallery studio-gallery" role="group" aria-label="Kamasada tetsubin and maker mark">
  <figure class="full-size">
    <a href="/tea/posts/tetsubin-history-2-morioka/kamasada-tetsubin.jpg" target="_blank" rel="noopener noreferrer" aria-label="Open the Kamasada Akinomi tetsubin image full size in a new tab">
      <img src="/tea/posts/tetsubin-history-2-morioka/kamasada-tetsubin.jpg" alt="Kamasada Akinomi large-arare tetsubin photographed from the front" loading="lazy" decoding="async" />
    </a>
    <figcaption>A Kamasada <em>Akinomi</em> large-arare tetsubin. Photo: <a href="https://fishingandcoffee.com/?p=7370">FISH&amp;CO.</a></figcaption>
  </figure>
  <figure class="full-size">
    <a href="/tea/posts/tetsubin-history-2-morioka/kamasada-mark.jpg" target="_blank" rel="noopener noreferrer" aria-label="Open the Kamasada body mark image full size in a new tab">
      <img src="/tea/posts/tetsubin-history-2-morioka/kamasada-mark.jpg" alt="Kamasada body mark below the spout reading Nambu Kamasada" loading="lazy" decoding="async" />
    </a>
    <figcaption>The same kettle below the spout. The raised mark reads <code>南部 釜定</code>, or <em>Nambu Kamasada</em>. Photo: <a href="https://fishingandcoffee.com/?p=7370">FISH&amp;CO.</a></figcaption>
  </figure>
</div>

## What the Four Workshops Show

The four workshops share a city and a craft name, but they do not work in the same way.

Suzuki Morihisa carries a domain lineage through a team workshop. Kunzan grew from apprenticeship into an independent house. Iwachu combines traditional kettle work with factory production. Kamasada gives one maker control over much of the process.

Their patterns overlap. Arare dots, rough skin, lines, and flower designs belong to a wider regional vocabulary. Start with the full object: body, spout, lid, handle, interior, seams, and mark. Then compare it with a complete workshop example.

A mark is useful when its reading, position, and date make sense together. A mark by itself is not enough. When the evidence stops, “Morioka tetsubin, maker uncertain” is a better description than a famous name chosen from one surface detail.

Next: [Mizusawa and Ōshū](/tea/tetsubin-history-3-mizusawa-oshu)

## Sources and Image Credits

The main regional sources are the [Nambu ironware association](https://www.ginga.or.jp/nanbu/making.html), [Morioka City](https://www.city.morioka.iwate.jp/_res/projects/default_project/_page_/001/024/890/2syou.pdf), [Traditional Crafts Aoyama Square](https://kougeihin.jp/craft/0701/), and Horie's [2019 Iwate University paper](https://www.jstage.jst.go.jp/article/sfj/70/5/70_255/_pdf/-char/ja).

Workshop histories, product pages, and mark sources are linked where they are used.

Previous: [The Birth of the Iron Kettle](/tea/tetsubin-history-1-birth-of-the-iron-kettle)
